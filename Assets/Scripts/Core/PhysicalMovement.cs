...
namespace Core
{
    ...
    public class PhysicalMovement : MonoBehaviour, IMovable
    {
        ...
        public void MoveOnRing()
        {
            ...
            Vector3 newposition = OrbitalMath.GetTangent(rb.position, ring.Item1.position, direction); //calculate new position
           
            //rb.MovePosition(new Vector3(newposition.x, rb.position.y,newposition.z)); //move to position
            rb.AddForce(new Vector3(newposition.x, rb.position.y,newposition.z) * moveForce, ForceMode.Force);
            Lookforward();
        }
        ...
        /*
         * keep the character align to the tangent and looking in the direction its facing.
         */
